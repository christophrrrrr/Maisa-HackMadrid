...
def evaluate(
    ...
    *,
    ...
    if missing:
        ...
        return _aggregate(invoice, findings, evidence)  # can't check anything else meaningfully
    ...
    return _aggregate(invoice, findings, evidence)


def _aggregate(invoice: InvoiceData, findings: list[Finding], evidence: dict) -> Outcome:
    if not findings:
